...
OCR_MAX_TOKENS=             # Default: 6000 (see the OCR note under Key Architectural Notes)
...
- **OCR is Groq Vision** (`qwen/qwen3.6-27b`), not EasyOCR or Tesseract. EasyOCR is **not** a dependency. Qwen is a *reasoning* model, so `ocr_with_groq()` passes `reasoning_format="hidden"` to keep `<think>` blocks out of the extracted text; non-reasoning models reject that parameter with a 400, so the call retries once without it. `OCR_MAX_TOKENS` (default **6000**) is squeezed between two limits: hidden reasoning is billed against it, so 4096 left roughly one page in three empty with `finish_reason="length"`; but Groq counts `max_tokens` toward the per-request TPM budget, and the free tier's 8000 TPM minus ~1834 prompt tokens for a rendered page leaves only ~6100 — 16384 is rejected with HTTP 413 before the model runs. Note the image cost is ~1834 tokens *regardless of render scale*, so lowering the PyMuPDF matrix trades OCR quality for nothing. A `NotFoundError` is logged with a distinct "model decommissioned" message, since otherwise a retired model makes scanned PDFs silently extract to empty text. It replaced `meta-llama/llama-4-scout-17b-16e-instruct`, which Groq decommissioned (404 `model_not_found`); Scout and Maverick are both gone, and Qwen is currently the only vision-capable model Groq serves.
